...
par(mar=c(7,4,2,0))
col <- colorRampPalette(c(icell8:::add.alpha(c("dodgerblue3", "slateblue","firebrick3"))))
...
kableExtra::kable(res, "html") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE) %>%
  kableExtra::column_spec(1, bold = T, width = "10em", color = icell8:::add.alpha("black", 0.7)) %>%
  kableExtra::column_spec(2, width = "8em", color = icell8:::add.alpha("black", 0.7)) %>%
  kableExtra::column_spec(3, width = "8em", color = icell8:::add.alpha("black", 0.7))
...
```{r heatmap, echo = FALSE, results = 'asis', fig.height = 6.69, fig.width = 10.00, fig.align = "center"}

load(file.path(params$data_dir, "ICELL8_correlation_data.rda"))

icell8::cor.heatmap(cm = correlation_data$cm, metadata = correlation_data$metadata,
...
icell8::cor.dist_boxplot(correlation_data$cor_dist$intracor_list,
                         cor_type = correlation_data$cor_type)
...
icell8::pca.plot(pca_obj = cluster_data$pca, metadata = cluster_data$metadata,
...
rots <- round(cluster_data$pca$rotation[,1:5], 2)

rots <- round(pca_obj$rotation[, 1:5], 2)

res <- apply(rots, 2, function(x) {
  
  head(names(sort(abs(x), decreasing = T)), 10)
  
})

kableExtra::kable(res, "html", caption = "Genes with Highest Impact on Principal Components 1-5") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE)
...
```{r tsne_app, echo = FALSE, results = 'asis', fig.height = 5.69, fig.width = 10.00, fig.align = "left"}

load(file.path(params$data_dir, "ICELL8_cluster_data.rda"))

icell8::tsne.plot(tsne_obj = cluster_data$tsne, metadata = cluster_data$metadata,
          grouping_var = cluster_data$grouping_var)
...
ge_analysis <- function(qc_data = NULL){
  
  res <- as.data.frame(t(apply(qc_data$gm[order(rowMeans(qc_data$gm), decreasing = T)[1:10], ], 1, function(x){
  
    c(mean(x), sd(x), median(x), min(x), max(x))
  
  })))

  colnames(res) <- c("mean", "sd", "median", "min", "max")

  res <- round(res, 2)

  t <- kableExtra::kable(res, "html") %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE) %>%
    kableExtra::column_spec(1, bold = T, width = "5em") %>%
    kableExtra::column_spec(2, width = "10em")
...
if(is.null(grouping_var) == TRUE){
  ...
  g = icell8:::gm.grouper(qc_data$metadata, grouping_var)
  
  if(length(levels(g$groups)) <= 10) {
    
    for(i in levels(g$groups)){
      
      gm <- qc_data$gm[, g$sort_order][, which(g$groups == i)]
      
      metadata <- qc_data$metadata[g$sort_order, ][which(g$groups == i), ]
      
      cat(paste0("### ", i, '\n'))
      
      ge_analysis(list(gm = gm, metadata = metadata))
